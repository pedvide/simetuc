...
class ExcTransition(Transition):
    ...
    def __init__(self, ion: IonType, state_i: int, state_f: int,
                 ...
                 label_ion: str = '', label_i: str = '', label_f: str = '') -> None:
        super(ExcTransition, self).__init__(ion, state_i, state_f, label_ion, label_i, label_f)
        self.active = active
    ...
    def __repr__(self) -> str:
        base_repr = super(ExcTransition, self).__repr__()
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if base_eq is False:
            return False
        for attr in ['active', 'degeneracy', 'pump_rate', 'power_dens', 't_pulse']:
            if getattr(self, attr) != getattr(other, attr):
                return False
        return True
...
class EneryTransferProcess():
    ...
    def __init__(self, transitions: Sequence[Transition],
        ...
        self.indices = list(itertools.chain.from_iterable(zip(*temp)))
